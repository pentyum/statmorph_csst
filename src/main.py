...
def run_sextractor(work_dir: str, detect_file: str, wht_file: str, use_existed: bool,
                   ...
                   config_values: Optional[Dict[str, Union[float, int, str]]] = None, command="source-extractor"):
    sextractor = SExtractor(work_dir, SExtractor.merge_sex_dict(SExtractor.CANDELS_UKIDSS_USF_CONFIG,
                            ...
                            SExtractor.OUTPUT_LIST_DEFAULT, command=command)
        ...
        if threads > 1:
            ...
            with ProcessPoolExecutor(threads) as exe:
                ...
                for label in run_labels:
                    ...
                    fs.append(
                        exe.submit(work_with_shared_memory, shm_img.name, shm_segm.name, shm_noise_name, segm_slice,
                                   label,
                                   shape, shm_img_cmp_name, output_image_dir, save_stamp_dir, set_centroid,
                                   set_asym_center,
...
def run_statmorph_stamp(catalog_file: str, save_file: str, threads: int, run_percentage: int, run_specified_label: int,
    ...
    if (threads > 1) and (len(run_rows) > 1):
        ...
        for block_i in range(len(run_rows_list)):
            ...
            logger.info("分段运行，当前段数%d/%d" % (block_i + 1, len(run_rows_list)))
    ...
    result_table: Table = Table.read(save_file, format="ascii")
    # result_table.sort("label")
    result_table = join(catalog_table, result_table, "label")
    result_table.write(save_file, format="ascii", overwrite=True)
...
help_str = """SExtractor-Statmorph_csst 简化合并版使用说明
	...
	-r, --stamp_catalog 如果填写则进入stamp模式，每个星系具有独立的stamp的fits文件，而不是从segmap中创建，stamp_catalog文件必须包含label，image_file_name，image_hdu_index，(segmap_file_name，segmap_hdu_index,noise_file_name，noise_hdu_index，cmp_file_name，cmp_hdu_index)列，如果不指定hdu_index，则默认为0。指定该项后，image_file、measure_file、wht_file、sextractor_work_dir、skip_sextractor将全部失效。
...
def main(argv) -> int:
    ...
    sextractor_cmd: str = config["sextractor_command"]
    ...
    if not check_not_null(sextractor_cmd):
        sextractor_cmd = "source-extractor"
    ...
    if stamp_catalog is None:
        sextractor = run_sextractor(sextractor_work_dir, detect_file, wht_file, skip_sextractor, measure_file,
                                    my_sextractor_config, command=sextractor_cmd)
